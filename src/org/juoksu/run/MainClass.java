...
public class MainClass {
	...
	private final String TABLE_HEADER = "\t<thead><td>Sij.</td><td>Numero</td><td>Nimi</td><td>Seura</td><td>Aika</td></thead>\n";
	...
	public void readSeriesFromFile(String filename) {
		...
		try {
			...
			while( (line = reader.readLine() ) != null ) {
				...
				sh.addSerie(series_parts[0], Integer.parseInt(series_parts[1]), Integer.parseInt(series_parts[2]));
			}
			reader.close();
		}
		catch (IOException e) {
	...
	public void writeHTML() {
		try {
			FileWriter fstream = new FileWriter("C:\\paimio-juoksu.html");
	...
	public void saveToFile(File file) throws IOException {
		...
		out.write("_SERIES\n");
		for (Serie s : sh.getSeries()) {
			System.out.println(s.getName() + ":" + s.getLow() + ":" + s.getHigh() +"\n");
			out.write(s.getName() + ":" + s.getLow() + ":" + s.getHigh() +"\n");
		}
		out.write("_RUNNERS\n");
		for (Serie s : sh.getSeries()) {
			for (Runner r : s.getRunners()) {
